...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      // Fetch user settings
      const settingsResponse = await apiClient.getSettings(user.id);
      if (settingsResponse.data) {
        setUserData(settingsResponse.data);
      }

      // Fetch dashboard stats
      const statsResponse = await apiClient.getDashboardStats(user.id);
      if (statsResponse.data) {
        setDashboardStats(statsResponse.data);
      }

      // Fetch card details
      const cardResponse = await apiClient.getCardDetails(user.id);
      if (cardResponse.data) {
        setCardDetails(cardResponse.data);
      }

      // Fetch recent transactions
      const transactionsResponse = await apiClient.getTransfers(user.id);
      ...
      <div className="flex-1 p-4 md:p-10 relative w-full overflow-hidden">
        ...
        <div className="mb-10 z-10 relative mt-8 md:mt-0">
          <h1 className="text-4xl md:text-5xl font-bold text-black mb-2">
            Welcome back, <span className="text-[#0000FF]">{userData?.first_name || 'User'}</span>
          </h1>
        ...
        <div className="relative z-10 mb-8">
          <div className="flex gap-4 md:gap-6 overflow-x-auto pb-4 md:pb-0 scrollbar-hide">
            {[
              {
                label: "TOTAL BALANCE",
                value: showBalance ? (dashboardStats ? `$ ${parseFloat(dashboardStats.total_balance).toLocaleString()}` : "$ 0.00") : "••••••",
                icon: Wallet,
                color: "from-blue-400 to-blue-600"
              },
              {
                label: "AVAILABLE BALANCE",
                value: showBalance ? (dashboardStats ? `$ ${parseFloat(dashboardStats.available_balance).toLocaleString()}` : "$ 0.00") : "••••••",
                icon: Wallet,
                color: "from-blue-500 to-cyan-600"
              },
              {
                label: "LOANS DUE",
                value: dashboardStats ? `$ ${parseFloat(dashboardStats.loans_due).toLocaleString()}` : "$ 0.00",
                icon: Banknote,
                color: "from-orange-400 via-red-500 to-pink-600"
              },
              {
                label: "MORTGAGES DUE",
                value: dashboardStats ? `$ ${parseFloat(dashboardStats.mortgage_due).toLocaleString()}` : "$ 0.00",
                icon: Banknote,
                color: "from-indigo-500 via-purple-500 to-pink-500"
              },
            ].map((card, idx) => (
              <div key={idx} className="scroll-animate stagger-item-1 group flex-shrink-0 w-64 md:w-auto md:flex-1">
          ...
          <div className="md:hidden absolute right-0 top-1/2 transform -translate-y-1/2 bg-white/90 rounded-full p-2 shadow-lg">
              ...
              <div className="relative h-72 bg-gradient-to-br from-blue-400 via-[#0000FF] to-blue-800 rounded-3xl p-8 shadow-2xl border border-white/20 overflow-hidden group cursor-pointer transform hover:scale-105 transition-all duration-300">
                ...
                <div className="relative z-10">
                  ...
                  <div className="text-white text-3xl font-mono tracking-widest mb-12 font-bold">
                    {cardDetails ? cardDetails.card_number.replace(/(\d{4})(?=\d)/g, '$1 ') : '•••• •••• •••• ••••'}
                  </div>

                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-white/70 text-xs uppercase mb-1">Card Holder</div>
                      <div className="text-white font-bold text-lg">{cardDetails?.card_holder_name || 'USER NAME'}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-white/70 text-xs uppercase mb-1">Expires</div>
                      <div className="text-white font-bold text-lg">{cardDetails?.card_expiry || 'MM/YY'}</div>
                    </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-[#0000FF] to-blue-600 text-white p-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group"
                  >
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-blue-100 to-blue-50 text-[#0000FF] p-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 border border-blue-200 flex items-center justify-center gap-2 group"
                  >
        ...
        {showComingSoon && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            />
            {/* Modal */}
            <div className="relative bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md mx-auto transform transition-all duration-300 scale-100 opacity-100">
              {/* Close Button */}
              <button
                onClick={() => setShowComingSoon(false)}
              ...
              {/* Content */}
              <div className="text-center">
                <div className="w-20 h-20 bg-gradient-to-br from-blue-400 to-[#0000FF] rounded-full flex items-center justify-center mx-auto mb-6">
                  <CreditCard size={32} className="text-white" />
                </div>
                <h2 className="text-2xl font-bold text-black mb-4">Coming Soon</h2>
                <p className="text-gray-600 mb-6">
                  The Pay Bills feature is currently under development. We're working hard to bring you this functionality soon!
                </p>
                <button
                  onClick={() => setShowComingSoon(false)}
                  className="w-full bg-gradient-to-r from-[#0000FF] to-blue-600 text-white py-3 px-6 rounded-xl font-semibold hover:shadow-lg transition-all duration-300"
                >
                  Got it!
                </button>
              </div>
